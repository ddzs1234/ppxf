...
#         ``start`` one needs to specify a pair of values ``[lower, upper]``.
    ...
    def __init__(self,dirf,templates, galaxy, noise, velscale, start,
                 bias=None, bounds=None, clean=False, component=0, degree=4,
                 fixed=None, fraction=None, ftol=1e-4, gas_component=None,
                 gas_names=None, gas_reddening=None, goodpixels=None, lam=None,
                 linear=False, mask=None, method='capfit', mdegree=0, moments=2,
...
################################################################################
    ...
    def plot(self):
        ...
        plt.plot(x, self.galaxy, 'k',label='galaxy')
        plt.plot(x[self.goodpixels], resid[self.goodpixels], 'd',
                 color='LimeGreen', mec='LimeGreen', ms=4,label='good pixel')
        plt.legend()
        ...
        if self.gas_any:
            ...
            plt.plot(x, self.gas_bestfit + mn, c='magenta', linewidth=2,label='gas_bestfit')
            plt.plot(x, self.bestfit, c='orange', linewidth=2,label='bestfit')
            plt.plot(x, stars_spectrum, 'r', linewidth=2,label='star-spectrum')
            plt.legend()
        else:
            plt.plot(x, self.bestfit, 'r', linewidth=2,label='bestfit')
            plt.plot(x[self.goodpixels], self.goodpixels*0 + mn, '.k', ms=1,label='goodpixel')
            plt.legend()
        plt.savefig(dirf[:-4]+'-ppxf.jpg',format='jpg',dpi=300)
        plt.axis('off')
        plt.close()
    ...
    def format_output(self, params, perror):
        ...
        if not self.quiet:
            ...
            print(("Best Fit:" + "{:>10}"*nmom).format(*txt),file=f)
            for j, (sol, mom) in enumerate(zip(self.sol, self.moments)):
                print((" comp. {}:" + "{:10.0f}"*2 + "{:10.3f}"*(mom - 2)).format(j, *sol),file=f)
            # reliable fitting result:
            cri=2/self.ndof
            if self.chi2>1-cri and self.chi2<1+cri:
                print(f"reliable(1sigma): ",file=f)
            elif self.chi2>1+2*cri or self.chi2<1-2*cri:
                print(f"warning(2sigma): ",file=f)
            else:
                print(f"reliable(2sigma): ",file=f)
            print(f"chi2/DOF: {self.chi2:.4g}",file=f)
            print(f"DOF: {self.ndof:.4g}",file=f)
            print(f"method = {self.method}; Jac calls: {self.njev}; "
                  f"Func calls: {self.nfev}; Status: {self.status}",file=f)
            nw = self.weights.size
            if self.reddening is not None:
                print(f"Stars Reddening E(B-V): {self.reddening:.3f}",file=f)
            if self.gas_reddening is not None:
                print(f"Gas Reddening E(B-V): {self.gas_reddening:.3f}",file=f)
            print(f"Nonzero Templates: {np.sum(self.weights > 0)} / {nw}",file=f)
            if self.weights.size <= 20:
                print('Templates weights:')
                print(("{:10.3g}"*self.weights.size).format(*self.weights),file=f)
        ...
        if self.gas_any:
            ...
            if not self.quiet:
                print('E(B-V)',self.gas_reddening,file=f)
                print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++',file=f)
                print('gas_component   name       flux       err      V     sig',file=f)
                print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++',file=f)
                for j, comp in enumerate(self.component[gas]):
                    print(f"Comp: %d  %12s %10.4g  %8.2g  %6.0f  %4.0f" %
                          (comp, self.gas_names[j], self.gas_flux[j],
                           self.gas_flux_error[j], *self.sol[comp][:2]),file=f)
        else:
