    ...
    for i in range(1,len(filename),1):
        ...
        if os.path.exists(dirname) and str(filename[i]) in sf_name:
            ...
            if tie_balmer==False and limit_doublets==False:
                dirpic='/home/zhaisai/zhaisai/result/3-3.5re/'+str(filename[i])+'.eps'
            elif tie_balmer==True and limit_doublets==True:
                dirpic='/home/zhaisai/zhaisai/result/3-3.5re/'+str(filename[i])+'-Balmer.eps'
    
            dirfile='/home/zhaisai/zhaisai/result/3-3.5re/'+str(filename[i])+'.txt'
            ...
            #
                ...
                for i in t.columns:
                    j+=1
                if j>=8:
                    flux=np.ma.array(t['col7'],mask=mask1)
