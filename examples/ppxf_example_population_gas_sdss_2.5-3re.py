    ...
    for i in range(1,len(filename),1):
        ...
        if os.path.exists(dirname) and str(filename[i]) in sf_name:
            ...
            if tie_balmer==False and limit_doublets==False:
                dirpic='/home/zhaisai/zhaisai/result/2.5-3re/'+str(filename[i])+'.eps'
            elif tie_balmer==True and limit_doublets==True:
                dirpic='/home/zhaisai/zhaisai/result/2.5-3re/'+str(filename[i])+'-Balmer.eps'
    
            dirfile='/home/zhaisai/zhaisai/result/2.5-3re/'+str(filename[i])+'.txt'
            ...
            #
                ...
                for i in t.columns:
                    j+=1
                if j>=7:
                    flux=np.ma.array(t['col6'],mask=mask1)
